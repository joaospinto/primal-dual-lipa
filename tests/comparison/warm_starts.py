    ...
    *,
    ...
    try:
        for t in range(T):
            x = np.asarray(
                problem.dynamics(
                    jnp.asarray(x), jnp.asarray(us[t]), theta_j, jnp.int32(t),
                ),
                dtype=np.float64,
            )
            if fallback_to_x_init and not np.all(np.isfinite(x)):
                X_init = np.asarray(
                    problem.X_init, dtype=np.float64,
...
def linspace_to_extracted_goal_warm_start(
    ...
    eq_T = np.asarray(
        problem.equalities(
            jnp.asarray(x0), u_zero, theta_j, jnp.int32(problem.T),
        ),
        dtype=np.float64,
    )
