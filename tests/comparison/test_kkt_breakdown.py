...
def _make_tiny_problem() -> ProblemSpec:
    ...
    return ProblemSpec(
        name="tiny",
        T=T, n=n, m=m, theta_dim=0,
        x0=jnp.array([1.0]),
        cost=cost, dynamics=dyn,
        equalities=None, inequalities=None,
        eq_dim=0, ineq_dim=0,
...
def _make_tiny_inequality_problem() -> ProblemSpec:
    ...
    return ProblemSpec(
        ...
        x0=jnp.array([1.0]),
        cost=cost, dynamics=dyn,
        equalities=None, inequalities=ineq,
        eq_dim=0, ineq_dim=1,
        X_init=jnp.zeros((T + 1, n)),
        U_init=jnp.zeros((T, m)),
        Theta_init=jnp.zeros((0,)),
...
class EvaluateProblemKKTTests(unittest.TestCase):
    ...
    def test_kkt_breakdown_no_constraints(self):
        ...
        out = evaluate_problem(
            problem, X, U, Theta,
            multipliers_eq=lam, multipliers_ineq=ineq_mults,
        )
    ...
    def test_kkt_init_violation(self):
        ...
        out = evaluate_problem(
            problem, X, U, Theta,
            multipliers_eq=lam, multipliers_ineq=np.zeros(0),
        )
        cost, eq_v, ineq_v, kkt = out
    ...
    def test_kkt_dual_and_complementarity_with_inequality(self):
        ...
        out = evaluate_problem(
            problem, X, U, Theta,
            multipliers_eq=lam, multipliers_ineq=z_mults,
        )
        _, _, _, kkt = out
        self.assertEqual(kkt.dual, 0.0)
        ...
        out2 = evaluate_problem(
            problem, X, U, Theta,
            multipliers_eq=lam, multipliers_ineq=z_mults_bad,
        )
        _, _, _, kkt2 = out2
...
class AttachKKTTests(unittest.TestCase):
    def test_attach_kkt_populates_optional_fields(self):
        result = SolverResult(
            solver_name="dummy", problem_name="dummy",
            iterations=0, solve_time_ms=0.0,
            final_cost=0.0, eq_violation_inf=0.0, ineq_violation_inf=0.0,
            success=True,
        )
        kkt = KKTBreakdown(
            init=0.1, dyn=0.2, eq=0.3, ineq=0.4,
            dual=0.5, complementarity=0.6, stationarity=0.7,
            joint=0.7,
        )
    ...
    def test_attach_kkt_propagates_nan_as_none(self):
        ...
        kkt = KKTBreakdown(
            init=0.1, dyn=0.2, eq=0.3, ineq=0.4,
            dual=float("nan"), complementarity=float("nan"),
