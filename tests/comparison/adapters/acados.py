...
from tests.comparison.warm_starts import (  # noqa: E402
    linspace_to_extracted_goal_warm_start as _linspace_warm_start,
    rollout_warm_start as _rollout_warm_start,
)
...
def _build_ocp(
    ...
    *,
    ...
    # qp_solver_mu0 is the initial barrier parameter HPIPM uses inside
    # the partial-condensing IPM. The acados default (1e3) over-
    # regularizes the QP at iter 1, which produces a long primal step
    # that the funnel L1 line search then has to retract — costing extra
    # SQP iterations on the larger problems (e.g. quadpendulum). A lower
    # mu0 lets HPIPM converge each QP closer to the active set on the
    # first SQP iteration; mu0=1e1 was the best across cartpole, acrobot
    # and quadpendulum in the sweep.
    ...
    # Tuning hook: apply caller-supplied overrides last so they win over
    # every default set above. Used by the tuning sweep driver via
    # ``--solver-kwargs-json '{"acados_extra_options": {...}}'``.
    # Each key is a ``solver_options`` attribute name; the value is set
    # directly. Unknown keys raise (rather than silently no-op) so typos
    # in tuning runs surface immediately.
    if extra_options:
        for k, v in extra_options.items():
            if not hasattr(ocp.solver_options, k):
                raise ValueError(
                    f"acados solver_options has no attribute {k!r} "
                    f"(extra_options keys must be valid acados knobs)",
                )
...
class AcadosAdapter(SolverAdapter):
    ...
    def __init__(
        ...
        max_iter: int = 1000,
        tol: float = 1e-6,
        acados_extra_options: dict | None = None,
    ) -> None:
        self.max_iter = max_iter
        self.tol = tol
        # Caller-supplied overrides on ``ocp.solver_options``. Applied
        # last in ``_build_ocp`` so they win over the adapter defaults.
        self.acados_extra_options = acados_extra_options or {}
    ...
    def solve(self, problem: ProblemSpec) -> SolverResult:
        ...
        # parallel solves.
        # Per-problem metadata layer (acados_settings) merges UNDER
        # self.acados_extra_options so CLI overrides still win over a
        # problem-baked tuning.
        merged_extra = {
            **problem.metadata.get("acados_settings", {}),
            **self.acados_extra_options,
        }
        with tempfile.TemporaryDirectory(prefix=f"acados_{problem.name}_") as scratch:
            ...
            try:
                with _chdir(scratch):
                    ...
                    # Warm start strategy. Default: forward rollout of
                    # U_init through problem.dynamics (see
                    # _rollout_warm_start docstring for why the LIPA-
                    # shipped X_init isn't always usable). When the
                    # problem opts in via
                    # ``metadata["acados_warm_start"] = "linspace"``,
                    # use the linspace-to-extracted-goal warm start
                    # instead (same logic as ipopt-casadi's "auto"
                    # strategy): for problems whose terminal equality
                    # encodes ``x_T = goal``, a straight linspace from
                    # ``x0`` to the inferred goal gives the SQP a
                    # near-feasible terminal point at iter 0 and
                    # avoids the funnel L1 line-search collapse seen
                    # when the rollout diverges into obstacles.
                    ws_kind = problem.metadata.get(
                        "acados_warm_start",
                        "rollout",
                    )
                    if ws_kind == "linspace":
                        xs_ws, us_ws = _linspace_warm_start(problem)
                    else:
                    ...
                    for i in range(T):
                        ...
                        pi_full.append(pi_i)
                    pi_full_arr = np.concatenate(pi_full) if pi_full else np.zeros(0)
