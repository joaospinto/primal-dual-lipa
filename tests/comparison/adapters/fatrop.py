...
def _build_opti(
    ...
    *,
    ...
    u_vars.append(
        ca.MX.zeros(m, 1)
    )  # dummy zero "control" at t=T (not an Opti variable)
    ...
    # Tuning-sweep overrides win over the defaults above.
    if fatrop_extra_options:
        fatrop_opts.update(fatrop_extra_options)
    plugin_opts = {
        "structure_detection": "manual",
        "nx": nx_list,
        "nu": nu_list,
...
def _remap_fatrop_lam_g(
    ...
    out_ineq = (
        np.zeros((T + 1) * ineq_dim, dtype=np.float64) if ineq_dim > 0 else np.zeros(0)
    )
...
class FatropAdapter(SolverAdapter):
    ...
    def __init__(
        ...
        self.tol = tol
        # See FatropMjxAdapter docstring; same convention for tuning
        # sweeps via ``--solver-kwargs-json``.
    ...
    def solve(self, problem: ProblemSpec) -> SolverResult:  # noqa: PLR0912, PLR0915
        ...
        # Per-problem metadata layer (fatrop_settings) merges UNDER
        # self.fatrop_extra_options so CLI overrides still win over a
        # problem-baked tuning.
        merged_fatrop_opts = {
            **problem.metadata.get("fatrop_settings", {}),
            **self.fatrop_extra_options,
        }
        try:
            opti, x_vars, u_vars, _, _, _, constraint_blocks = _build_opti(
                ...
                tol=tol,
                fatrop_extra_options=merged_fatrop_opts,
            )
        except Exception as e:  # noqa: BLE001
            return make_failure_result(
                self.name,
        ...
        if lam_g_final.size > 0:
            ...
            multipliers_ineq = (
                np.zeros((T + 1) * problem.ineq_dim)
                if problem.ineq_dim > 0
                else np.zeros(0)
            )
