...
class TrajaxAdapter(SolverAdapter):
    ...
    def __init__(
        ...
        # CLI-side tuning hook: a dict matching the same keys as
        # ``problem.metadata["trajax_settings"]`` (max_iter, tol,
        # maxiter_al, psd_delta, penalty_init, penalty_update_rate,
        # alpha_min). Applied AFTER the per-problem metadata overrides
        # so a tuning sweep can shadow a problem-baked tuning. Mirrors
        # ``ipopt_extra_options`` / ``sip_extra_settings``.
        trajax_extra_settings: Optional[dict] = None,
    ) -> None:
        ...
        self.trajax_extra_settings = dict(trajax_extra_settings or {})
    ...
    def solve(self, problem: ProblemSpec) -> SolverResult:  # noqa: PLR0915
        ...
        # Per-problem schedule overrides via problem.metadata["trajax_settings"]
        # (mirrors lipa_settings / sip_settings pattern). Unknown keys are
        # silently ignored; only the named knobs below are honored. The
        # constructor-side ``trajax_extra_settings`` (CLI tuning hook)
        # is applied LAST so a tuning sweep can shadow the per-problem
        # bake.
        from tests.comparison.problem_spec import effective_solver_tol

        overrides = dict(problem.metadata.get("trajax_settings", {}))
        overrides.update(self.trajax_extra_settings)
        max_iter = int(overrides.get("max_iter", self.max_iter))
        # Honour per-problem ``success_tol`` (e.g. 1e-3 for MJX) as the
        # default — the trajax_settings / extra-settings overrides still
        # win if explicitly set.
        tol = float(overrides.get("tol", effective_solver_tol(problem, self.tol)))
        maxiter_al = int(overrides.get("maxiter_al", self.maxiter_al))
        psd_delta = float(overrides.get("psd_delta", self.psd_delta))
        penalty_init = float(overrides.get("penalty_init", self.penalty_init))
        penalty_update_rate = float(
            overrides.get("penalty_update_rate", self.penalty_update_rate),
        )
        alpha_min = float(overrides.get("alpha_min", self.alpha_min))
        ...
        try:
            if has_constraints:
                eq_for_call = (
                    tj_eq
                    if tj_eq is not None
                    else (lambda x, u, t: jnp.zeros(1, dtype=jnp.float64))
                )
                ineq_for_call = (
                    tj_ineq
                    if tj_ineq is not None
                    else (lambda x, u, t: -jnp.ones(1, dtype=jnp.float64))
                )
        ...
        try:
            if has_constraints:
                ...
                success = grad_norm <= tol or iterations < max_iter
        ...
        multipliers_eq = (
            np.zeros(eq_full_size, dtype=np.float64) if eq_full_size > 0 else None
        )
        multipliers_ineq = (
            np.zeros(ineq_full_size, dtype=np.float64) if ineq_full_size > 0 else None
        )
        if trajax_dual_eq is not None and eq_dim > 0 and multipliers_eq is not None:
            try:
                ...
                pass
        if (
            trajax_dual_ineq is not None
            and ineq_dim > 0
            and multipliers_ineq is not None
        ):
            try:
