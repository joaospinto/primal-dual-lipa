...
Unlike IPOPT (which we run with ``hessian_approximation='limited-memory'``),
...
* ``metadata.get("is_mjx") != True``: fatrop has the analytical
  adapter (``fatrop.py``) for those.
...
# Forward-rollout warm-start (with non-finite fallback to X_init) lives
# in tests.comparison.warm_starts; see that module's docstring for the
# rationale.
from tests.comparison.warm_starts import rollout_warm_start  # noqa: E402


def _initial_iterate(problem: ProblemSpec) -> tuple[np.ndarray, np.ndarray]:
    """Pick (X_ws, U_ws). Try dynamics rollout first, fall back to X_init.

    See the module docstring. ``U_ws`` is always the LIPA-shipped
    ``problem.U_init`` since the rollout uses it.
    """
    return rollout_warm_start(problem, fallback_to_x_init=True)
...
class FatropMjxAdapter(SolverAdapter):
    ...
    def solve(self, problem: ProblemSpec) -> SolverResult:  # noqa: PLR0912, PLR0915
        ...
        if not problem.metadata.get("is_mjx"):
            return make_failure_result(
                ...
                "fatrop-mjx adapter is MJX-only; this problem has no "
                "metadata['is_mjx'] flag. Use the 'fatrop' adapter for "
    ...
    def _solve(self, problem: ProblemSpec) -> SolverResult:  # noqa: PLR0912, PLR0915
        ...
        def dyn_adj_hess(z, seed, t):
            ...
            return jax.jacfwd(jax.jacrev(
                lambda zz: jnp.dot(seed, dyn_eval(zz, t))
            ))(z)
        ...
        def cost_adj_hess_xu(z, seed, t):
            return jax.jacfwd(jax.jacrev(
                lambda zz: jnp.dot(seed, cost_eval_xu(zz, t))
            ))(z)
        ...
        def cost_adj_hess_x(x, seed, t):
            return jax.jacfwd(jax.jacrev(
                lambda xx: jnp.dot(seed, cost_eval_x(xx, t))
            ))(x)
        ...
        if has_ineq:
            ...
            def ineq_adj_hess_xu(z, seed, t):
                return jax.jacfwd(jax.jacrev(
                    lambda zz: jnp.dot(seed, ineq_eval_xu(zz, t))
                ))(z)
            ...
            def ineq_adj_hess_x(x, seed, t):
                return jax.jacfwd(jax.jacrev(
                    lambda xx: jnp.dot(seed, ineq_eval_x(xx, t))
                ))(x)
        ...
        # Terminal stage (t = T): cost & ineq take only x, no control.
        ...
        for k in range(K):
            ...
            # Path constraints: equality (init defect at k=0) + inequality.
            eq_rows: list = []
            ineq_rows: list = []
            if k == 0:
            ...
            if ineq_rows:
                ...
                constraint_blocks.append({
                    "kind": "ineq", "stage": k, "size": int(n_ineq),
                })
        ...
        try:
            ...
            try:
                lam_g_final = np.asarray(
                    opti.debug.value(opti.lam_g), dtype=np.float64,
                ).reshape(-1)
            except Exception:  # noqa: BLE001
                lam_g_final = np.zeros(0)
        ...
        if lam_g_final.size > 0:
            ...
            for blk in constraint_blocks:
                ...
                if kind == "init":
                    ...
                    multipliers_ineq[stage * problem.ineq_dim:
                                     stage * problem.ineq_dim + sz] = slice_lam
