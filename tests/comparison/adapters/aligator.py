...
def _ensure_aligator_on_path() -> bool:
    ...
    if any(m in sys.modules for m in ("pinocchio", "eigenpy", "crocoddyl", "mim_solvers")):
        return False
...
def _build_casadi_callbacks(problem: ProblemSpec, t: int, terminal: bool):
    ...
    if next_x_sx is None:
        ...
        dyn_jac_x_fn = ca.Function("dyn_jx", [x_sx, u_sx], [ca.jacobian(next_x_sx, x_sx)])
        dyn_jac_u_fn = ca.Function("dyn_ju", [x_sx, u_sx], [ca.jacobian(next_x_sx, u_sx)])
    ...
    if ineq_sx is None or ineq_sx.numel() == 0:
        ...
        ineq_jac_x_fn = ca.Function("ineq_jx", [x_sx, u_sx], [ca.jacobian(ineq_sx, x_sx)])
        ineq_jac_u_fn = ca.Function("ineq_ju", [x_sx, u_sx], [ca.jacobian(ineq_sx, u_sx)])
...
def _build_jax_callbacks(problem: ProblemSpec, t: int, terminal: bool):
    ...
    return {
        ...
        "cost_hess_xu": jax.jit(
            jax.jacobian(jax.grad(_cost_xu, argnums=0), argnums=1)
        ),
        "dyn": jax.jit(_dyn_xu),
...
def _build_stage_cost(aligator, space, problem, t: int, terminal: bool, backend: str = "jax"):
...
def _build_constraint_function(aligator, space, problem, t: int, terminal: bool, kind: str, backend: str = "jax"):
...
class AligatorAdapter(SolverAdapter):
    ...
    def solve(self, problem: ProblemSpec) -> SolverResult:
        ...
        if problem.metadata.get("is_mjx"):
        ...
        be = self.backend
        dyn_models = [_build_stage_dynamics(aligator, space, problem, t, backend=be) for t in range(T)]
        cost_models = [_build_stage_cost(aligator, space, problem, t, terminal=False, backend=be) for t in range(T)]
        eq_models = [_build_constraint_function(aligator, space, problem, t, terminal=False, kind="eq", backend=be) for t in range(T)]
        ineq_models = [_build_constraint_function(aligator, space, problem, t, terminal=False, kind="ineq", backend=be) for t in range(T)]

        term_cost = _build_stage_cost(aligator, space, problem, T, terminal=True, backend=be)
        term_eq = _build_constraint_function(aligator, space, problem, T, terminal=True, kind="eq", backend=be)
        term_ineq = _build_constraint_function(aligator, space, problem, T, terminal=True, kind="ineq", backend=be)
        ...
        if term_eq is not None:
            traj_problem.addTerminalConstraint(term_eq, constraints.EqualityConstraintSet())
        ...
        verbose_level = (
            aligator.VerboseLevel.VERBOSE if self.verbose else aligator.VerboseLevel.QUIET
        )

        def _make_solver():
            s = aligator.SolverProxDDP(
                self.tol, self.mu_init,
                max_iters=self.max_iter, verbose=verbose_level,
            )
        ...
        try:
            ...
            if hasattr(res, "lams"):
                try:
                    lams_raw = [np.asarray(L, dtype=np.float64).reshape(-1) for L in res.lams]
