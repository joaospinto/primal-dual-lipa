    ...
    class _JaxModel(crocoddyl.ActionModelAbstract):
        def __init__(self_inner):  # noqa: N804
            crocoddyl.ActionModelAbstract.__init__(
                self_inner, state, m, max(n, 1), ng, nh,
            )
            self_inner._t = t_j
            if ng > 0:
                lb = np.empty(ng)
        ...
        def calc(self_inner, data, x, u=None):  # noqa: N805
            ...
            if terminal:
                ...
                if ng > 0:
                    pieces = []
                    if ng_ineq > 0:
                        pieces.append(np.asarray(fns["ineq"](x_np, tau), dtype=np.float64).reshape(-1))
                    if ng_eq > 0:
                        pieces.append(np.asarray(fns["eq"](x_np, tau), dtype=np.float64).reshape(-1))
                ...
                data.xnext = np.asarray(fns["dyn"](x_np, u_np, tau), dtype=np.float64).reshape(-1)
                if ng > 0:
                    pieces = []
                    if ng_ineq > 0:
                        pieces.append(np.asarray(fns["ineq"](x_np, u_np, tau), dtype=np.float64).reshape(-1))
                    if ng_eq > 0:
                        pieces.append(np.asarray(fns["eq"](x_np, u_np, tau), dtype=np.float64).reshape(-1))
                    data.g = np.concatenate(pieces) if pieces else np.empty(0)
        ...
        def calcDiff(self_inner, data, x, u=None):  # noqa: N805
            ...
            tau = self_inner._t
            if terminal:
                data.Lx = np.asarray(fns["cost_grad_x"](x_np, tau), dtype=np.float64).reshape(-1)
                data.Lxx = np.asarray(fns["cost_hess_xx"](x_np, tau), dtype=np.float64).reshape(n, n)
                if ng > 0:
                    gx_pieces = []
                    if ng_ineq > 0:
                        gx_pieces.append(np.asarray(fns["ineq_jac_x"](x_np, tau), dtype=np.float64).reshape(ng_ineq, n))
                    if ng_eq > 0:
                        gx_pieces.append(np.asarray(fns["eq_jac_x"](x_np, tau), dtype=np.float64).reshape(ng_eq, n))
                ...
                if u is None:
                    ...
                    u_np = np.asarray(u, dtype=np.float64).reshape(-1)
                data.Lx = np.asarray(fns["cost_grad_x"](x_np, u_np, tau), dtype=np.float64).reshape(-1)
                data.Lxx = np.asarray(fns["cost_hess_xx"](x_np, u_np, tau), dtype=np.float64).reshape(n, n)
                data.Lu = np.asarray(fns["cost_grad_u"](x_np, u_np, tau), dtype=np.float64).reshape(-1)
                data.Luu = np.asarray(fns["cost_hess_uu"](x_np, u_np, tau), dtype=np.float64).reshape(m, m)
                data.Lxu = np.asarray(fns["cost_hess_xu"](x_np, u_np, tau), dtype=np.float64).reshape(n, m)
                data.Fx = np.asarray(fns["dyn_jac_x"](x_np, u_np, tau), dtype=np.float64).reshape(n, n)
                data.Fu = np.asarray(fns["dyn_jac_u"](x_np, u_np, tau), dtype=np.float64).reshape(n, m)
                if ng > 0:
                    gx_pieces = []
                    gu_pieces = []
                    if ng_ineq > 0:
                        gx_pieces.append(np.asarray(fns["ineq_jac_x"](x_np, u_np, tau), dtype=np.float64).reshape(ng_ineq, n))
                        gu_pieces.append(np.asarray(fns["ineq_jac_u"](x_np, u_np, tau), dtype=np.float64).reshape(ng_ineq, m))
                    if ng_eq > 0:
                        gx_pieces.append(np.asarray(fns["eq_jac_x"](x_np, u_np, tau), dtype=np.float64).reshape(ng_eq, n))
                        gu_pieces.append(np.asarray(fns["eq_jac_u"](x_np, u_np, tau), dtype=np.float64).reshape(ng_eq, m))
                    data.Gx = np.vstack(gx_pieces) if gx_pieces else np.empty((0, n))
...
def _make_casadi_action_model(problem: ProblemSpec, t: int, terminal: bool):
    ...
    * ``L_xx_xu`` = full ``hessian(L, [x; u])`` is (n+m)x(n+m) symmetric.
    ...
    g_expr = ca.vertcat(ineq_padded, eq_padded) if (ng_ineq + ng_eq) > 0 else ca.SX.zeros(0, 1)
    Gx_expr = ca.jacobian(g_expr, x_sx) if g_expr.numel() > 0 else ca.SX.zeros(0, n)
    Gu_expr = ca.jacobian(g_expr, u_sx) if (g_expr.numel() > 0 and m_full > 0) else ca.SX.zeros(0, m_full)
    ...
    der_fn = ca.Function(
        ...
        inputs,
        [Lx_expr, Lu_expr, Lxx_expr, Luu_expr, Lxu_expr, Fx_expr, Fu_expr, Gx_expr, Gu_expr],
        ["x", "u"],
    ...
    class _CasadiModel(crocoddyl.ActionModelAbstract):
        def __init__(self_inner):  # noqa: N804
            crocoddyl.ActionModelAbstract.__init__(
                self_inner, state, m, max(n, 1), ng, 0,
            )
            if ng > 0:
                lb = np.empty(ng)
                ub = np.empty(ng)
...
class CsqpAdapter(SolverAdapter):
    ...
    def solve(self, problem: ProblemSpec) -> SolverResult:
        ...
        if self.backend == "casadi":
            ...
            running_models = [
                _make_casadi_action_model(problem, t, terminal=False) for t in range(problem.T)
            ]
            terminal_model = _make_casadi_action_model(problem, problem.T, terminal=True)
        else:
            ...
            terminal_model = _make_jax_action_model(
                problem, problem.T, terminal=True, shared=shared,
            )
        ...
        def _make_solver():
            ...
            s.termination_tolerance = self.tol
            return s
        ...
        try:
            ...
            existing_cbs = list(solver.getCallbacks()) if hasattr(solver, "getCallbacks") else []
        ...
        try:
            ...
            if lag_mul_arr.shape[0] == T_problem + 1 and lag_mul_arr.shape[1] == n:
                ...
                # X[k]), negate.
                multipliers_eq[n:n + T_problem * n] = (
                    -lag_mul_arr[1:].reshape(-1)
                )
            # path-constraint multipliers (per-stage [ineq (ng_ineq); eq (ng_eq)])
            ineq_full_size = (T_problem + 1) * ineq_dim
            multipliers_ineq = np.zeros(ineq_full_size, dtype=np.float64) if ineq_dim > 0 else np.zeros(0)
            if hasattr(solver, "y"):
                for t in range(T_problem + 1):
                    ...
                    if ineq_dim > 0:
                        multipliers_ineq[t * ineq_dim:(t + 1) * ineq_dim] = (
                            y_t[:ineq_dim]
                        )
                    if eq_dim > 0:
                        base = n + T_problem * n
