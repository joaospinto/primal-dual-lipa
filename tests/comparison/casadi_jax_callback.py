...
class PerStageJaxCallback:
    ...
    def _build_forward_only(self, name: str, in_dim: int, out_dim: int):
        ...
        class _Cb(ca.Callback):
            ...
            def get_sparsity_in(cb_self, _i):
                return ca.Sparsity.dense(in_dim, 1)  # noqa: N805

            def get_sparsity_out(cb_self, _i):
                return ca.Sparsity.dense(out_dim, 1)  # noqa: N805
            ...
            def has_jacobian(cb_self):
                return True  # noqa: N805
    ...
    def _build_with_reverse(self, name: str, in_dim: int, out_dim: int):
        ...
        class _Cb(ca.Callback):
            ...
            def has_reverse(cb_self, nadj):
                return nadj == 1  # noqa: N805, ARG002
...
def _build_jac_cb(wrapper, name: str, in_dim: int, out_dim: int, jac_opts: dict):
    ...
    class _JacCb(ca.Callback):
        ...
        def get_n_in(jac_self):
            return 2  # noqa: N805

        def get_n_out(jac_self):
            return 1  # noqa: N805
...
def _build_rev_cb(wrapper, name: str, in_dim: int, out_dim: int):
    ...
    class _RevCb(ca.Callback):
        ...
        def get_n_in(rev_self):
            return 3  # noqa: N805

        def get_n_out(rev_self):
            return 1  # noqa: N805
...
def _build_rev_jac_cb(wrapper, name: str, in_dim: int, out_dim: int):
    ...
    class _RevJacCb(ca.Callback):
        ...
        def get_sparsity_in(jac_self, i):  # noqa: N805
            if i == 0:
                return ca.Sparsity.dense(in_dim, 1)  # z
            if i == 1:
                return ca.Sparsity.dense(out_dim, 1)  # y
            if i == 2:
                return ca.Sparsity.dense(out_dim, 1)  # seed
            return ca.Sparsity.dense(in_dim, 1)  # adj (rev's nominal out)

        def get_sparsity_out(jac_self, i):  # noqa: N805
            if i == 0:
                return ca.Sparsity.dense(in_dim, in_dim)  # d adj / d z
            if i == 1:
                return ca.Sparsity.dense(in_dim, out_dim)  # d adj / d y
