...
def _ode(x, u):
    ...
    dM_dphi = jnp.array(
        [
            ...
            [0.0, 0.0, 0.0, 0.0],
            [
                -MASS_POLE * L_POLE * jnp.sin(phi),
                MASS_POLE * L_POLE * jnp.cos(phi),
                0.0,
                0.0,
            ],
        ]
...
def _inequalities(x, u, theta, t):
    ...
    obs_cons = jnp.concatenate(
        [_per_obs(oc, orad) for oc, orad in zip(obs_centers, obs_radii)]
    )
...
def _casadi_builder_factory(with_theta: bool):
    def _builder(x_sx, u_sx, theta_sx, t):
        ...
        ctrl_cost = ca.dot(
            u_sx - ca.DM(np.asarray(U_HOVER)), u_sx - ca.DM(np.asarray(U_HOVER))
        )
        stage_cost = WEIGHTS[0] * pos_cost + WEIGHTS[1] * ctrl_cost
        ...
        pos = q[:2]
        Rmat = ca.SX(2, 2)
        Rmat[0, 0] = ca.cos(theta_q)
        Rmat[0, 1] = -ca.sin(theta_q)
        Rmat[1, 0] = ca.sin(theta_q)
        Rmat[1, 1] = ca.cos(theta_q)
        offsets = ca.SX(5, 2)
        ...
        for i in range(4):
            ...
            centers[i, 1] = world[1]
        pole_tip = ca.vertcat(
            pos[0] + L_POLE * ca.sin(phi), pos[1] - L_POLE * ca.cos(phi)
        )
        ...
        metadata={
            ...
            "aligator_casadi_settings": {
                ...
                "bcl_params": {
                    ...
                    "mu_lower_bound": 1e-12,
                    "prim_alpha": 0.1,
                    "prim_beta": 0.9,
                },
                "reg_init": 1e-2,
                "reg_min": 1e-6,
                "linear_solver_choice": "LQ_SOLVER_STAGEDENSE",
            },
            # Lift the CSQP iter cap so the outer SQP loop has room to
            # converge on quadpendulum (mim_solvers' inner ADMM needs
            # quite a few outer SQP iterations on this 4-link problem).
            "max_iter_overrides": {"csqp": 1000},
            "csqp_settings": {
                "filter_size": 10,
            },
            "csqp_jax_settings": {
            ...
            # CasADi backend doesn't tolerate the 1e-9 inner tolerance
            # (the inner ADMM diverges); the framework tol=1e-6 is
            ...
            "ipopt_settings": (
                ...
                if not with_theta
                else {"alpha_for_y": "bound-mult"}
            ),
            "trajax_settings": dict(
                penalty_init=50.0,
                penalty_update_rate=3.0,
                maxiter_al=30,
                psd_delta=1e-3,
            ),
            # acados's default rollout warm start (U_HOVER through the
            # nonlinear dynamics from x0) cancels gravity and leaves
            # x at x0, so the terminal equality residual stays O(1)
            # and the funnel L1 line search collapses to alpha_min at
            # a feasible-but-far-from-goal iterate. The linspace
            # warm start (x0 -> inferred goal) handles the terminal
            # equality at iteration 0 and lets SQP_WITH_FEASIBLE_QP
            # converge from a much closer initial point.
            "acados_warm_start": "linspace",
        },
    )
