...
def _cost(x, u, theta, t):  # noqa: ARG001
    ...
    stagewise = 0.5 * COST_PARAMS[1] * jnp.dot(delta, delta) + 0.5 * COST_PARAMS[
        2
    ] * jnp.dot(u, u)
...
def _casadi_builder(x_sx, u_sx, theta_sx, t):  # noqa: ARG001
    ...
    ddth2 = (a + d2 / d1 * phi1 - m2 * l1 * lc2 * dth1**2 * ca.sin(th2) - phi2) / (
        m2 * lc2**2 + I2 - d2**2 / d1
    )
    ddth1 = -(d2 * ddth2 + phi1) / d1
    ...
    stagewise = 0.5 * float(COST_PARAMS[1]) * ca.dot(delta, delta) + 0.5 * float(
        COST_PARAMS[2]
    ) * ca.dot(u_sx, u_sx)
